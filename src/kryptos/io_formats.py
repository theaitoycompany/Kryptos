...
def build_document(
    ...
    for i, turn in enumerate(turns):
        text = unicodedata.normalize("NFC", (turn.text or "")).strip()
        if not text:
            continue
        turn.text = text
...
def _assign_word_offsets(turn: Turn) -> None:
    ...
    for w in turn.words:
        ...
        if not token:
            continue
        idx = low.find(token.lower(), cursor)
        if idx < 0:
            ...
            if idx < 0:
                continue
            token = stripped
        w.char_start = turn.char_start + idx
        w.char_end = turn.char_start + idx + len(token)
...
def parse_json(obj: Any, doc_id: str = "") -> Document:
    ...
    raw_turns = (
        obj.get("turns") or obj.get("segments") or obj.get("utterances") or obj.get("results") or []
    )
    ...
    for seg in raw_turns:
        if not isinstance(seg, dict):
            continue
        text = seg.get("text") or seg.get("transcript") or seg.get("utterance") or ""
        speaker = (
            seg.get("speaker")
        ...
        words = []
        for w in seg.get("words") or seg.get("tokens") or []:
...
def parse_srt_vtt(text: str, doc_id: str = "") -> Document:
    ...
    def flush() -> None:
        if buf:
            ...
            m = re.match(r"^\s*(?:<v\s+([^>]+)>|([A-Z][A-Za-z0-9 _'\-]{0,30})\s*:)\s*(.*)$", body)
            if m:
                speaker = (m.group(1) or m.group(2) or speaker).strip()
                body = m.group(3)
    ...
    for line in text.splitlines():
        s = line.strip()
        if not s or s.upper().startswith("WEBVTT") or _SRT_INDEX.match(s):
            if not s:
                flush()
            continue
